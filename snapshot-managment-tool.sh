...
btrfs_managment() {
  ...
  example: subvolume or subvolume/another subvolume"
  cd folder
  read -e answr2
  cd ..
  clear
...
modify_boot() {
  ...
  cat $time.conf | grep options | grep subvol && sed -i 's/\<subvol='$answr3'\>//g' $time.conf
  sed -i '/rw/ s/$/ subvol='$time'/' $time.conf
  cd /boot
  mkdir $time
  check_for_another_folder
}
modify_boot_with_own_name() {
  ...
  cat $name_for_snapshot.conf | grep options | grep subvol && sed -i 's/\<subvol='$answr3'\>//g' $name_for_snapshot.conf
  sed -i '/rw/ s/$/ subvol='$name_for_snapshot'/' $name_for_snapshot.conf
  cd /boot
